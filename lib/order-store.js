...
const ORDER_RETENTION_MS = 90 * 864e5;
const MAX_ORDER_HISTORY_PER_ACCOUNT = 5;
...
class OrderStore {
  constructor(directory, options = {}) {
    ...
    // Node ships SQLite itself: deployment must not depend on a separately built addon.
    // Keep the existing file/schema and the previous driver's five-second busy timeout.
    this.db = new DatabaseSync(this.file, { timeout: 5000, allowExtension: false });
    this.db.exec(`
    ...
    this.db.exec(
      `PRAGMA max_page_count = ${Math.floor((options.maxBytes || MAX_DB_BYTES) / pageSize)}`,
    );
  ...
  list(account, since = 0) {
    return this.db
      .prepare(
        "SELECT * FROM orders WHERE account = ? AND created > ? ORDER BY created DESC LIMIT 50",
      )
      .all(account, since)
      .map((row) => this.decode(row));
  }
  pruneAccountHistory(account, keep = MAX_ORDER_HISTORY_PER_ACCOUNT) {
    // Never remove a live checkout: a delayed payment still needs its original
    // invoice so it can be verified and credited safely. Terminal invoices are
    // compact history only, and each account keeps just its newest five.
    const result = this.db.prepare(`
      DELETE FROM orders WHERE id IN (
        SELECT id FROM (
          SELECT id, ROW_NUMBER() OVER (ORDER BY created DESC, id DESC) AS position
          FROM orders
          WHERE account = ? AND (fulfilled IS NOT NULL OR status IN (4, 5, 7))
        ) WHERE position > ?
      )
    `).run(account, keep);
    if (result.changes) this.db.exec("PRAGMA incremental_vacuum(16)");
    return Number(result.changes);
  }
  pruneExcessHistory(keep = MAX_ORDER_HISTORY_PER_ACCOUNT, limit = 500) {
    const result = this.db.prepare(`
      DELETE FROM orders WHERE id IN (
        SELECT id FROM (
          SELECT id, ROW_NUMBER() OVER (
            PARTITION BY account ORDER BY created DESC, id DESC
          ) AS position
          FROM orders
          WHERE fulfilled IS NOT NULL OR status IN (4, 5, 7)
        ) WHERE position > ? LIMIT ?
      )
    `).run(keep, limit);
    if (result.changes) this.db.exec("PRAGMA incremental_vacuum(64)");
    return Number(result.changes);
  }
  prune(now = Date.now(), limit = 500) {
    // Delete only invoice details. Coupon receipts and account entitlements are
    // separate: removing history must never allow another redemption or debit time.
    const result = this.db.prepare(
      "DELETE FROM orders WHERE id IN (SELECT id FROM orders WHERE created <= ? ORDER BY created LIMIT ?)",
    ).run(now - ORDER_RETENTION_MS, limit);
    if (result.changes) this.db.exec("PRAGMA incremental_vacuum(256)");
    return Number(result.changes);
  }
  ...
  assertCapacity() {
    ...
    if ((pages - freePages) * pageSize + active * 4096 + 64_000_000 >= this.ceiling)
      throw Object.assign(
  ...
  put(order) {
    ...
    const terminal = completed || ["failed", "expired", "coupon_reused"].includes(order.localStatus);
    ...
    this.pruneAccountHistory(order.accountId);
  }
  close() {
    this.db.close();
  }
